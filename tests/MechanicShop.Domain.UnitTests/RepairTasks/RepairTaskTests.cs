...
public class RepairTaskTests
{
    ...
    public void Create_WithValidData_ShouldSucceed()
    {
        ...
        const RepairDurationInMinute estimatedDurationInMin = RepairDurationInMinute.Min30;
        ...
        var result =
            RepairTask.Create(
                    ...
                    name: name,
                    laborCost: laborCost,
                    repairDurationInMinute: estimatedDurationInMin,
                    parts: parts);
        ...
        Assert.Equal(estimatedDurationInMin, task.RepairDurationInMinute);
    ...
    public void Create_WithEmptyName_ShouldFail()
    {
        ...
        var result = RepairTask.Create(
                    ...
                    name: name,
                    laborCost: 100,
                    repairDurationInMinute: RepairDurationInMinute.Min30,
                    parts: [PartFactory.CreatePart().Value]);

        Assert.True(result.IsError);

        Assert.Equal(RepairTaskError.NameRequired.Code, result.TopError.Code);
    }
    ...
    public void Create_WithInvalidLaborCost_ShouldFail()
    {
        ...
        Assert.Equal(RepairTaskError.LaborCostInvalid.Code, result.TopError.Code);
    }

    [Fact]
    public void Create_WithInvalidDuration_ShouldFail()
    {
        const RepairDurationInMinute invalidDurationValue = (RepairDurationInMinute)999;

        var result = RepairTask.Create(
            id: Guid.NewGuid(),
            name: "Brake Inspection",
            laborCost: 100,
            repairDurationInMinute: invalidDurationValue,
            parts: [PartFactory.CreatePart().Value]);

        Assert.True(result.IsError);

        Assert.Equal(RepairTaskError.DurationInvalid.Code, result.TopError.Code);
    }
    ...
    public void Update_ShouldReturnSuccess_WithValidValues()
    {
        ...
        var result = task.Update("Valid", 123m, RepairDurationInMinute.Min30);

        Assert.True(result.IsSuccess);
        Assert.Equal("Valid", task.Name);
        Assert.Equal(123m, task.LaborCost);
        Assert.Equal(RepairDurationInMinute.Min30, task.RepairDurationInMinute);
    }

    [Theory]
    [InlineData("", 1, RepairDurationInMinute.Min30, false)]
    [InlineData("  ", 1, RepairDurationInMinute.Min30, false)]
    [InlineData("Name", 0, RepairDurationInMinute.Min30, false)]
    [InlineData("Name", 10001, RepairDurationInMinute.Min30, false)]
    public void Update_ShouldReturnError_ForInvalidNameOrCost(string name, decimal cost, RepairDurationInMinute dur, bool expected)
    {
        var task = RepairTaskFactory.CreateRepairTask().Value;
    ...
    {
        var task = RepairTaskFactory.CreateRepairTask().Value;
        const RepairDurationInMinute invalid = (RepairDurationInMinute)999;
